...
class NegSampleDataLoader(AbstractDataLoader):
    ...
    def _neg_sampling(self, inter_feat):
        if self.neg_sample_args['strategy'] == 'by':#可看出这样的参数基本只适用userid,itemid这样的交互类型数据，不适合seq，用于seq回踩杨处负样本作为正例数据，l酸依这应该就是为什么loss很难降低，因为加入了错误样本
        ...
        else:#'strategy'为'none'或者full,用于seq合适，
            return inter_feat
