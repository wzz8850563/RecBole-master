...
class GNN(nn.Module):
    ...
    def GNNCell(self, A, hidden):#注意hidden虽然是max_session_len，但是不是按原始seq顺序的,而是np.unique(u_input)这个顺序，所以每个位置都是不同的向量
        ...
        """
        #经过一个门出来为 batch,max_session_len,embedding_size,每个item为图上相邻item embding的和
        ...
        # (batch_size, max_session_len, embedding_size)
        i_r, i_i, i_n = gi.chunk(3, 2) #即公式Wz*a Wr*a Wo*a
        h_r, h_i, h_n = gh.chunk(3, 2) #即公式 Uz*v Ur*v Uo*v
        #sigmoid后还是(batch_size, max_session_len, embedding_size)，相当于每个元素都是不同权重，不是每个向量不同权重
        reset_gate = torch.sigmoid(i_r + h_r)
        input_gate = torch.sigmoid(i_i + h_i)
        new_gate = torch.tanh(i_n + reset_gate * h_n)
        hy = (1 - input_gate) * hidden + input_gate * new_gate
        #hy 为(batch_size, max_session_len, embedding_size)
        return hy
...
class SRGNN(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        hidden = self.gnn(A, hidden)#此时hidden不是序列顺序，是np.unique(u_input)的顺序，即唯一表示一个向量
        #每个seq位置从1维变成self.embedding_size维，且扩增维度数都和seq位置数的值一样，和hidden一样都是(batch_size, max_session_len, embedding_size)
        alias_inputs = alias_inputs.view(-1, alias_inputs.size(1), 1).expand(-1, -1, self.embedding_size)
        seq_hidden = torch.gather(hidden, dim=1, index=alias_inputs)#将gnn表示好的向量按seq顺序排列
        # fetch the last hidden state of last timestamp
        ht = self.gather_indexes(seq_hidden, item_seq_len - 1)
        q1 = self.linear_one(ht).view(ht.size(0), 1, ht.size(1))#batch,1,embedding_size
        q2 = self.linear_two(seq_hidden)#batch,seqlen,embedding_size
        
        
        #相当于q1和q2每个向量分别相加结合，结果batch,seqlen,embedding_size，但是linear_three处理后为batch,seqlen
        #每个位置只有一个权重，这样就方便将所有向量加权结合了，类似attention
        alpha = self.linear_three(torch.sigmoid(q1 + q2))
        a = torch.sum(alpha * seq_hidden * mask.view(mask.size(0), -1, 1).float(), 1)#排除mask的向量加权求和，batch,1,emdding_size
